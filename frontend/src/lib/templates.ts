...
export type TemplateTag = "premium" | "dark" | "light" | "minimal" | "modern";
...
export const TEMPLATES = [
  {
    ...
    premium: true,
    tags: ["premium", "dark", "modern"] as TemplateTag[],
...
`,
  },
  {
    ...
    premium: true,
    tags: ["premium", "light", "minimal"] as TemplateTag[],
    template: `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
`,
  },
  {
    ...
    premium: true,
    tags: ["premium", "light", "modern"] as TemplateTag[],
    template: `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
`,
  },
  {
    ...
    style: "Dark Terminal",
    premium: false,
    tags: ["dark", "minimal"] as TemplateTag[],
    template: `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
`,
  },
  {
    ...
    premium: false,
    tags: ["dark", "modern"] as TemplateTag[],
    template: `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
    ...
    <div class="hero-inner">
      <div class="hero-content">
        ...
        <div class="hero-socials">
          <a href="{{github}}" target="_blank" rel="noopener noreferrer" aria-label="GitHub">GH</a>
          <a href="{{linkedin}}" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">LI</a>
  ...
  <section class="section" id="contact">
    <div class="container">
      ...
      <div class="contact-grid">
        ...
        <a href="{{linkedin}}" target="_blank" rel="noopener noreferrer" class="contact-card">
          <div class="contact-icon">in</div>
          <div><div class="contact-label">LinkedIn</div><div class="contact-value">{{linkedin}}</div></div>
        </a>
        <a href="{{github}}" target="_blank" rel="noopener noreferrer" class="contact-card">
          <div class="contact-icon">&lt;/&gt;</div>
...
`,
  ...
  {
    id: "clean-light-unique",
    name: "Clean Light Unique",
    style: "Light Minimal",
    premium: false,
    tags: ["light", "minimal"] as TemplateTag[],
    template: `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<style>
  *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
  :root {
    --bg: #fafaf9; --surface: #ffffff; --border: #e8e5e0;
    --text: #1a1a1a; --muted: #6b6b6b; --accent: #2563eb;
    --accent-light: rgba(37,99,235,0.08); --shadow: 0 1px 3px rgba(0,0,0,0.06);
  ...
  .hero-name { font-family: 'Space Grotesk', sans-serif; font-size: clamp(2.2rem, 5vw, 3.5rem); font-weight: 700; letter-spacing: -0.03em; margin-bottom: 0.5rem; }
  .hero-title { font-size: 1.1rem; color: var(--accent); font-weight: 500; margin-bottom: 1rem; }
  .hero-summary { font-size: 1rem; color: var(--muted); max-width: 520px; margin: 0 auto; line-height: 1.7; }
  .hero-links { display: flex; justify-content: center; gap: 1.5rem; margin-top: 1.5rem; flex-wrap: wrap; }
  .hero-links a { font-size: 0.85rem; color: var(--accent); text-decoration: none; font-weight: 500; padding: 0.4rem 1rem; border: 1px solid var(--accent); border-radius: 6px; transition: all 0.2s; }
  .hero-links a:hover { background: var(--accent); color: #fff; }

  /* Sections */
  .section { padding: 2.5rem 0; border-top: 1px solid var(--border); }
  .section-label { font-family: 'Space Grotesk', sans-serif; font-size: 0.7rem; font-weight: 600; letter-spacing: 0.15em; text-transform: uppercase; color: var(--accent); margin-bottom: 1.25rem; }
  .section-title { font-family: 'Space Grotesk', sans-serif; font-size: 1.5rem; font-weight: 700; margin-bottom: 1rem; }

  /* About */
  .about-text { font-size: 0.95rem; color: var(--muted); line-height: 1.8; }

  /* Skills */
  .skills-wrap { display: flex; flex-wrap: wrap; gap: 0.5rem; }
  .skill-chip { background: var(--accent-light); color: var(--accent); font-size: 0.82rem; font-weight: 500; padding: 0.4rem 0.9rem; border-radius: 20px; }

  /* Projects */
  .projects-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 1rem; }
  .project-card { background: var(--surface); border: 1px solid var(--border); border-radius: 12px; padding: 1.5rem; transition: box-shadow 0.2s, transform 0.2s; }
  .project-card:hover { box-shadow: var(--shadow-hover); transform: translateY(-2px); }
  .project-name { font-family: 'Space Grotesk', sans-serif; font-size: 1.05rem; font-weight: 600; margin-bottom: 0.5rem; }
  .project-desc { font-size: 0.88rem; color: var(--muted); line-height: 1.6; }

  /* Education */
  .edu-list { display: flex; flex-direction: column; gap: 0.75rem; }
  .edu-item { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 1.25rem 1.5rem; }
  .edu-title { font-weight: 600; font-size: 0.95rem; margin-bottom: 0.25rem; }
  .edu-detail { font-size: 0.85rem; color: var(--muted); }

  /* Experience */
  .exp-list { display: flex; flex-direction: column; gap: 0.75rem; }
  .exp-item { border-left: 3px solid var(--accent); padding-left: 1.25rem; }
  .exp-role { font-weight: 600; font-size: 0.95rem; margin-bottom: 0.25rem; }
  .exp-detail { font-size: 0.85rem; color: var(--muted); line-height: 1.6; }

  /* Contact */
  .contact-row { display: flex; flex-wrap: wrap; gap: 1rem; }
  .contact-item { background: var(--surface); border: 1px solid var(--border); border-radius: 10px; padding: 1rem 1.5rem; text-decoration: none; color: var(--text); transition: border-color 0.2s; flex: 1; min-width: 200px; }
  .contact-item:hover { border-color: var(--accent); }
  .contact-label { font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.1em; color: var(--muted); margin-bottom: 0.25rem; }
  .contact-value { font-weight: 600; font-size: 0.9rem; word-break: break-all; }

  /* Footer */
  footer { border-top: 1px solid var(--border); padding: 2rem 0; text-align: center; font-size: 0.8rem; color: var(--muted); margin-top: 2rem; }
...
<div class="wrapper">
  ...
  <section class="section" id="skills">
    ...
    <div class="skills-wrap" id="skills-container"></div>
  </section>
  ...
  <section class="section" id="contact">
    ...
    <div class="contact-row">
      ...
      <a class="contact-item" href="{{linkedin}}" target="_blank" rel="noopener noreferrer">
        <div class="contact-label">LinkedIn</div>
        <div class="contact-value">{{linkedin}}</div>
      </a>
      <a class="contact-item" href="{{github}}" target="_blank" rel="noopener noreferrer">
  ...
  <footer>Built with AI Portfolio Maker</footer>
</div>
...
<script>
  ...
  (function() {
    ...
    entries.forEach(function(entry) {
      ...
      var item = document.createElement('div');
      item.className = 'exp-item';
      item.innerHTML = '<div class="exp-role">' + role + '</div>' +
        (detail ? '<div class="exp-detail">' + detail + '</div>' : '');
      container.appendChild(item);
    });
  })();
...
`,
...

  {
    ...
    premium: false,
    tags: ["light", "modern"] as TemplateTag[],
    template: `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
