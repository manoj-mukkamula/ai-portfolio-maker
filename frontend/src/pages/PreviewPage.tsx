...
// Premium redesign: polished navbar, smooth skeleton-to-content transition,
// working PDF/GitHub/download/share actions, fullscreen toggle.
...
const SHIMMER = `
  ...
  .ps-shimmer {
    background: linear-gradient(90deg, var(--ps-base, rgba(148,163,184,0.10)) 0%, var(--ps-highlight, rgba(148,163,184,0.20)) 50%, var(--ps-base, rgba(148,163,184,0.10)) 100%);
    background-size: 800px 100%;
    animation: preview-shimmer 1.6s ease-in-out infinite;
    border-radius: 0.5rem;
  }
  .dark .ps-shimmer {
    --ps-base: rgba(255,255,255,0.05);
    --ps-highlight: rgba(255,255,255,0.10);
  }
`;

const SkBlock = ({ w = "100%", h = "1rem", r = "0.5rem", delay = 0 }: { w?: string; h?: string; r?: string; delay?: number }) => (
  <div className="ps-shimmer" style={{ width: w, height: h, borderRadius: r, animationDelay: `${delay}ms` }} />
);
  ...
  <>
    ...
    <div className="w-full bg-background" style={{ minHeight: "calc(100vh - 56px)" }}>
      ...
      <div className="flex flex-col items-center justify-center gap-5 py-24 px-8"
        ...
        <SkBlock w="96px" h="96px" r="50%" />
        <SkBlock w="260px" h="28px" />
        <SkBlock w="380px" h="16px" delay={60} />
        <div className="flex gap-3 mt-2">
          <SkBlock w="88px" h="34px" r="8px" delay={80} />
          <SkBlock w="88px" h="34px" r="8px" delay={140} />
          <SkBlock w="88px" h="34px" r="8px" delay={200} />
        </div>
      </div>
      {/* Body sections */}
      <div className="max-w-4xl mx-auto w-full px-6 py-12 space-y-12">
        ...
        <div className="space-y-3">
          <SkBlock w="140px" h="20px" />
          <SkBlock w="100%" h="13px" delay={40} />
          <SkBlock w="92%" h="13px" delay={80} />
          <SkBlock w="76%" h="13px" delay={120} />
        </div>
        {/* Skills */}
        <div className="space-y-4">
          <SkBlock w="100px" h="20px" />
          <div className="flex flex-wrap gap-2">
            {[80, 72, 96, 60, 84, 68, 76, 88].map((w, i) => (
              <SkBlock key={i} w={`${w}px`} h="30px" r="15px" delay={i * 30} />
            ))}
          </div>
        </div>
        {/* Projects */}
        <div className="space-y-4">
          <SkBlock w="120px" h="20px" />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="rounded-xl border border-border p-5 space-y-3">
                <SkBlock w="65%" h="16px" delay={i * 50} />
                <SkBlock w="100%" h="11px" delay={i * 50 + 40} />
                <SkBlock w="85%" h="11px" delay={i * 50 + 80} />
                <div className="flex gap-2 pt-1">
                  <SkBlock w="56px" h="22px" r="4px" />
                  <SkBlock w="48px" h="22px" r="4px" />
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="bg-card border border-border rounded-2xl shadow-modal w-full max-w-lg p-7 animate-fade-in">
      ...
      <ol className="space-y-3">
        {[
          ...
          { n: "02", t: 'Create a new GitHub repository named yourname.github.io, replacing "yourname" with your actual GitHub username.' },
      ...
      <header className="h-14 shrink-0 flex items-center justify-between px-4 lg:px-6 border-b border-border bg-card sticky top-0 z-40"
        ...
        <div className="flex items-center gap-1">
          ...
          >
            ...
            <span className="hidden md:inline ml-0.5">PDF</span>
          </button>
          ...
          >
            ...
            <span className="hidden md:inline ml-0.5">Host</span>
          </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold text-white transition-all hover:opacity-90 hover:-translate-y-px active:scale-[0.98] ml-1"
            style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)" }}
          >
