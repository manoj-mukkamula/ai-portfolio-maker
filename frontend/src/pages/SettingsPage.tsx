            ...
            {[
              ...
              <div key={field.label} className="flex items-center justify-between py-4 border-b border-border last:border-0">
                <div>
                  ...
                  <p className="text-base font-semibold text-foreground mt-0.5">{field.value}</p>
                </div>
            ...
            <div className="text-right">
              ...
              <div className="w-28 h-2 rounded-full bg-border overflow-hidden mt-2">
                <div
                  ...
                  style={{
                    width: `${(credits / 5) * 100}%`,
                    background: credits === 0 ? "#ef4444" : credits <= 2 ? "#f59e0b" : "#6366f1",
                  }}
                />
              </div>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                {theme === "dark" ? "Easier on the eyes in low light" : "Better for well-lit environments"}
              </p>
            </div>
            <button
              onClick={toggleTheme}
        ...
        <section className="bg-card border border-destructive/25 rounded-2xl p-6 shadow-card">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Irreversible actions — proceed carefully</p>
            </div>
          </div>
          <div className="flex items-center justify-between p-5 rounded-xl border border-destructive/20 bg-destructive/5">
            <div>
              <p className="text-base font-semibold text-foreground">Delete my account</p>
              <p className="text-sm text-muted-foreground mt-0.5">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl shadow-modal w-full max-w-md mx-4 p-8 animate-slide-up">
            <div className="flex items-center justify-between mb-7">
              <div>
                <h3 className="text-xl font-bold text-foreground">Sorry to see you go</h3>
                <p className="text-sm text-muted-foreground mt-0.5">Help us understand why you're leaving</p>
              </div>
              <button onClick={handleCloseModal} className="p-2 rounded-xl hover:bg-secondary transition-colors">
                <X className="w-5 h-5 text-muted-foreground" />
              </button>
            </div>
            <p className="text-base font-medium text-foreground mb-4">Why are you deleting your account?</p>
              ...
              {FEEDBACK_REASONS.map((reason) => (
                ...
                >
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                    feedbackReason === reason ? "border-primary bg-primary" : "border-border"
                  }`}>
                    {feedbackReason === reason && <Check className="w-3 h-3 text-white" />}
                  </div>
                  {reason}
                </button>
              ))}
            ...
            <div className="flex gap-3">
              <button onClick={handleCloseModal} className="flex-1 py-3 rounded-xl border border-border text-base font-medium hover:bg-secondary transition-colors">
                Cancel
              </button>
              <button
          ...
          <div className="bg-card border border-border rounded-2xl shadow-modal w-full max-w-sm mx-4 p-8 animate-slide-up">
            <div className="flex items-center gap-3 mb-7">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Enter your password to proceed</p>
              </div>
              <button onClick={handleCloseModal} className="ml-auto p-2 rounded-xl hover:bg-secondary transition-colors">
                <X className="w-5 h-5 text-muted-foreground" />
              </button>
            </div>
            <p className="text-base text-muted-foreground mb-6 leading-relaxed">
              This will permanently delete your account and all{" "}
              <span className="font-semibold text-foreground">portfolios, settings, and data</span>.
            </p>
            <div className="mb-6">
